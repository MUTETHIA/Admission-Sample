 ...
 $intake =$_GET['intake'];
 $level = $_GET['level'];
   if(!empty(($level) &&($intake))){
 $check = $conn->prepare("SELECT applicants_details.*, applicants_course_details.Mode_Of_Study, programmes.programme, admissiondates.intake_name,campuses.Campus_Name, course_levels.Level_Name
...
WHERE ( applicants_course_details.intake=$intake AND applicants_course_details.Level_id =$level)");
       $check->execute();

       $check2 = $conn->prepare("SELECT applicants_details.*, applicants_course_details.Mode_Of_Study, programmes.programme, admissiondates.intake_name,faculty.faculty_name,campuses.Campus_Name, course_levels.Level_Name
FROM applicants_course_details
    INNER JOIN applicants_details ON (applicants_course_details.Email_Address = applicants_details.Email_Address)
    INNER JOIN programmes ON (applicants_course_details.Course_id = programmes.code)
    INNER JOIN admissiondates ON (applicants_course_details.intake = admissiondates.intake_id)
...
<?php include_once('../inc/header1.php');  ?>
  ...
  <div class="row">
	...
	<div class="row"> <div class="col-lg-4">
      <a href="GenerateIntakeLevelProgrammesPdf?intake=<?php echo $intake;?>&level=<?php echo $level;?>" type="button" class="btn btn-danger"><span class="glyphicon glyphicon-arrow-right"> </span>&nbsp;Export to PDF</a>
    </div> </div><br/>

                        <div class="panel panel-primary">
                            <div class="panel-heading"><span class="glyphicon glyphicon-education"></span>&nbsp;&nbsp;Applicants in <?php echo $name['Level_Name']; ?> LEVEL Record list</div>
